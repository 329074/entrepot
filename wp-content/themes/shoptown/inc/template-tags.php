 ...
 */
if ( ! function_exists( 'shoptown_paging_nav' ) ) :
/**
...
function shoptown_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'shoptown' ); ?></h1>
...
function tm_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'shoptown' ); ?></h1>
		<div class="nav-links">
			<?php
			if ( is_attachment() ) :
				previous_post_link( '%link',  wp_kses( __( '<span class="meta-nav">Published In</span>%title', 'shoptown' ),shoptown_allowed_html()) );
			else :
				previous_post_link( '%link', wp_kses( __( '<i class="fa fa-chevron-left"></i>%title', 'shoptown' ),shoptown_allowed_html()) );
				next_post_link( '%link', wp_kses( __( '%title<i class="fa fa-chevron-right"></i>', 'shoptown' ),shoptown_allowed_html()) );
			endif;
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
...
function tm_posted_on() {
	if ( is_sticky() && is_home() && ! is_paged() ) {
		echo '<span class="featured-post">' . esc_html__( 'Sticky', 'shoptown' ) . '</span>';
	}
...
function tm_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'shoptown_category_count' ) ) ) {
		...
		set_transient( 'shoptown_category_count', $all_the_cool_cats );
	}
...
function tm_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'shoptown_category_count' );
}
