...
class tm_Featured_Content {
	...
	public static function init() {
		...
		// Return early if "tm_Featured_Content_filter" has not been defined.
		if ( ! isset( $theme_support[0]['tm_Featured_Content_filter'] ) ) {
			return;
		}
		$filter = $theme_support[0]['tm_Featured_Content_filter'];
	 ...
	 *
	 * Sets the "tm_Featured_Content_ids" transient.
	 *
	 * @static
	 * @access public
	 * @since TemplateMela 1.0
	...
	public static function get_featured_post_ids() {
		...
		$featured_ids = get_transient( 'tm_Featured_Content_ids' );
		...
		set_transient( 'tm_Featured_Content_ids', $featured_ids );
		return $featured_ids;
	}
	/**
	 ...
	 *
	 * @see tm_Featured_Content::validate_settings().
	 *
	 * @static
	 * @access public
	 * @since TemplateMela 1.0
	...
	public static function delete_transient() {
		delete_transient( 'tm_Featured_Content_ids' );
	}
	/**
	 ...
	 * the transient is deleted by tm_Featured_Content::validate_settings().
	 *
	 ...
	 *
	 * @uses tm_Featured_Content::get_setting()
	 */
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'tm_Featured_Content', array(
			...
			'description'    => sprintf( __( 'Use a <a href="%1$s">tag</a> to feature your posts. If no posts match the tag, <a href="%2$s">sticky posts</a> will be displayed instead.', 'shoptown' ),
				esc_url( add_query_arg( 'tag', _x( 'featured', 'featured content default tag slug', 'shoptown' ), admin_url( 'edit.php' ) ) ),
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'    => esc_html__( 'Tag Name', 'shoptown' ),
			'section'  => 'tm_Featured_Content',
			'priority' => 20,
		) );
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'    => esc_html__( 'Don&rsquo;t display tag on front end.', 'shoptown' ),
			'section'  => 'tm_Featured_Content',
			'type'     => 'checkbox',
			'priority' => 30,
		) );
	 ...
	 * delete the transient set in tm_Featured_Content::get_tm_Featured_Content().
	 *
	 * @static
	 * @access public
	 * @since TemplateMela 1.0
